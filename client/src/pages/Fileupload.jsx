...
const FileUpload = () => {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/v1/materials/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-6">
      <div
        className={`w-40 p-10 border-2 border-dashed ${dragActive ? 'border-black' : 'border-gray-300'} text-center`}
